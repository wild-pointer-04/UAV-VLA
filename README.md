...
本项目同时是吕鹤冉同学 2026 届本科毕业设计的实现工程，在原始研究 UAV-VLA (HRI 2025) 的基础上额外构建了**软件工程层**——以 Flask + SQLite 为底座的 Web 应用，将算法管线封装为面向最终用户的可视化交互系统。
...
├── run_vlm_yuan.py                   # 原版 (论文复刻) VLM 调用版本
├── generate_plans.py                 # ★ 调用 GLM-4 生成任务脚本
├── generate_plans_yuan.py            # 原版 (论文复刻) 任务规划版本
│
...
$$
\text{lat} = \text{nw\_lat} - \frac{y\%}{100} \times (\text{nw\_lat} - \text{se\_lat})
$$

$$
\text{lon} = \text{nw\_lon} + \frac{x\%}{100} \times (\text{se\_lon} - \text{nw\_lon})
$$
